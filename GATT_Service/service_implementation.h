...
typedef struct {
	...
	uint16_t  	characteristic_f_client_configuration ;
	char *  	characteristic_f_user_description ;

	// Characteristic G information
	char *  	characteristic_g_value ;
	uint16_t  	characteristic_g_client_configuration ;
	char *  	characteristic_g_user_description ;
	...
	uint16_t 	characteristic_f_client_configuration_handle ;
	uint16_t 	characteristic_f_user_description_handle ;

	// Characteristic G handles
	uint16_t  	characteristic_g_handle ;
	uint16_t 	characteristic_g_client_configuration_handle ;
	uint16_t 	characteristic_g_user_description_handle ;
	...
	btstack_context_callback_registration_t callback_f ;
	btstack_context_callback_registration_t callback_g ;
...
char characteristic_a[] = "Time" ;
char characteristic_b[] = "Current State" ;
char characteristic_c[] = "Requested State" ;
char characteristic_d[] = "Potassium" ;
char characteristic_e[] = "pH" ;
char characteristic_f[] = "Lactate" ;
char characteristic_g[] = "Cycle Counter" ;
...
static void characteristic_f_callback(void * context) {
	// Associate the void pointer input with our custom service object
	custom_service_t * instance = (custom_service_t *) context ;
	// Send a notification
	att_server_notify(instance->con_handle, instance->characteristic_f_handle, instance->characteristic_f_value, strlen(instance->characteristic_f_value));
}

static void characteristic_g_callback(void * context) {
	// Associate the void pointer input with our custom service object
	custom_service_t * instance = (custom_service_t *) context ;
	// Send a notification
	att_server_notify(instance->con_handle, instance->characteristic_g_handle, instance->characteristic_g_value, strlen(instance->characteristic_g_value));
}
...
static uint16_t custom_service_read_callback(hci_con_handle_t con_handle, uint16_t attribute_handle, uint16_t offset, uint8_t * buffer, uint16_t buffer_size){
	...
	if (attribute_handle == service_object.characteristic_f_client_configuration_handle){
		return att_read_callback_handle_little_endian_16(service_object.characteristic_f_client_configuration, offset, buffer, buffer_size);
	}
	...
	// Characteristic G
	if (attribute_handle == service_object.characteristic_g_handle){
		return att_read_callback_handle_blob(service_object.characteristic_g_value, strlen(service_object.characteristic_g_value), offset, buffer, buffer_size);
	}
	if (attribute_handle == service_object.characteristic_g_client_configuration_handle){
		return att_read_callback_handle_little_endian_16(service_object.characteristic_g_client_configuration, offset, buffer, buffer_size);
	}
	if (attribute_handle == service_object.characteristic_g_user_description_handle) {
		return att_read_callback_handle_blob(service_object.characteristic_g_user_description, strlen(service_object.characteristic_g_user_description), offset, buffer, buffer_size);
	}
...
static int custom_service_write_callback(hci_con_handle_t con_handle, uint16_t attribute_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size){
	...
	// Enable/disable notificatins
	if (attribute_handle == service_object.characteristic_f_client_configuration_handle){
		service_object.characteristic_f_client_configuration = little_endian_read_16(buffer, 0);
		service_object.con_handle = con_handle;
	}

	// Enable/disable notificatins
	if (attribute_handle == service_object.characteristic_g_client_configuration_handle){
		service_object.characteristic_g_client_configuration = little_endian_read_16(buffer, 0);
		service_object.con_handle = con_handle;
	}
...
void custom_service_server_init(char * a_ptr, char * b_ptr, char * c_ptr, char * d_ptr, char * e_ptr, char * f_ptr, char * g_ptr){
	...
	instance->characteristic_g_value = g_ptr ;
	...
	instance->characteristic_g_user_description = characteristic_g ;
	...
	instance->characteristic_g_handle = ATT_CHARACTERISTIC_0000FF17_0000_1000_8000_00805F9B34FB_01_VALUE_HANDLE ;
	instance->characteristic_g_user_description_handle = ATT_CHARACTERISTIC_0000FF17_0000_1000_8000_00805F9B34FB_01_USER_DESCRIPTION_HANDLE ;
...
void set_characteristic_a_value(float value) {
	...
	// Update field value
	sprintf(instance->characteristic_a_value, "%f", value) ;
...
// Update Characteristic B value
void set_characteristic_b_value(int value) {
	// Pointer to our service object
	custom_service_t * instance = &service_object ;

	// Update field value
	sprintf(instance->characteristic_b_value, "%d", value) ;
...
void set_characteristic_c_value(int value) {
	// Pointer to our service object
	custom_service_t * instance = &service_object ;

	// Update field value
	sprintf(instance->characteristic_c_value, "%d", value) ;
...
// Update Characteristic D value
void set_characteristic_d_value(float value) {
	// Pointer to our service object
	custom_service_t * instance = &service_object ;

	// Update field value
	sprintf(instance->characteristic_d_value, "%f", value) ;
...
// Update Characteristic E value
void set_characteristic_e_value(float value) {
	// Pointer to our service object
	custom_service_t * instance = &service_object ;

	// Update field value
	sprintf(instance->characteristic_e_value, "%f", value) ;
...
// Update Characteristic F value
void set_characteristic_f_value(float value) {
	// Pointer to our service object
	custom_service_t * instance = &service_object ;

	// Update field value
	sprintf(instance->characteristic_f_value, "%f", value) ;

	// Are notifications enabled? If so, register a callback
	if (instance->characteristic_f_client_configuration){
		instance->callback_f.callback = &characteristic_f_callback;
		instance->callback_f.context  = (void*) instance;
		att_server_register_can_send_now_callback(&instance->callback_f, instance->con_handle);
	}
}

// Update Characteristic B value
void set_characteristic_g_value(int value) {
	// Pointer to our service object
	custom_service_t * instance = &service_object ;

	// Update field value
	sprintf(instance->characteristic_g_value, "%d", value) ;

	// Are notifications enabled? If so, register a callback
	if (instance->characteristic_g_client_configuration){
		instance->callback_g.callback = &characteristic_g_callback;
		instance->callback_g.context  = (void*) instance;
		att_server_register_can_send_now_callback(&instance->callback_g, instance->con_handle);
	}
}


// Example function using pointer to character buffer
void set_characteristic_z_value(char * c_ptr) {
